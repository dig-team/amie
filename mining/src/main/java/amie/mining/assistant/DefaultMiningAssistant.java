...
public class DefaultMiningAssistant extends MiningAssistant {
	...
	/**
	 * Helper method that implements the logic to produce new rules with an
	 * additional closing atom
					...
					if (variable != sourceVariable) {
						...
						if (this.enabledFunctionalityHeuristic && this.enableQueryRewriting) {
							...
							if (rewrittenQuery == null) {
								...
								promisingRelations = kb.countProjectionBindings(rule.getHead(), rule.getAntecedent(),
										newEdge[1]);
								long t2 = System.currentTimeMillis();
								if ((t2 - t1) > 20000 && this.verbose)
									System.err.println("countProjectionBindings var=" + kb.unmap(newEdge[1]) + " "
											+ rule + " has taken " + (t2 - t1) + " ms");
							} else {
								System.out.println(rewrittenQuery + " is a rewrite of " + rule);
								long t1 = System.currentTimeMillis();
								promisingRelations = kb.countProjectionBindings(rewrittenQuery.getHead(),
										rewrittenQuery.getAntecedent(), newEdge[1]);
								long t2 = System.currentTimeMillis();
								if ((t2 - t1) > 20000 && this.verbose)
									System.err.println(
											"countProjectionBindings on rewritten query var=" + kb.unmap(newEdge[1])
													+ " " + rewrittenQuery + " has taken " + (t2 - t1) + " ms");
							}
						} else {
							promisingRelations = this.kb.countProjectionBindings(rule.getHead(), rule.getAntecedent(),
									newEdge[1]);
						}
		...
		for (int joinPosition : joinPositions) {
			for (int joinVariable : joinVariables) {
				...
				if (rewrittenQuery == null) {
					...
					long t2 = System.currentTimeMillis();
					if ((t2 - t1) > 20000 && this.verbose) {
						System.err.println("countProjectionBindings var=" + kb.unmap(newEdge[1]) + " " + rule
								+ " has taken " + (t2 - t1) + " ms");
					}
				} else {
					long t1 = System.currentTimeMillis();
					promisingRelations = this.kb.countProjectionBindings(rewrittenQuery.getHead(),
							rewrittenQuery.getAntecedent(), newEdge[1]);
					long t2 = System.currentTimeMillis();
					if ((t2 - t1) > 20000 && this.verbose)
						System.err.println("countProjectionBindings on rewritten query var=" + kb.unmap(newEdge[1])
								+ " " + rewrittenQuery + " has taken " + (t2 - t1) + " ms");
				}
				...
				IntList listOfPromisingRelations = decreasingKeys(promisingRelations);
				// The relations are sorted by support, therefore we can stop once we have
				// reached
				// the minimum support.
				for (int relation : listOfPromisingRelations) {
					int cardinality = promisingRelations.get(relation);
					...
					if (!recursiveAtoms.isEmpty()) {
						if (canAddInstantiatedAtoms()) {
							...
							if (boundHead) {
								...
								finalCardinality = this.kb.countDistinct(candidate.getFunctionalVariable(),
										candidate.getTriples());
							} else {
								// Still pending
								finalCardinality = this.kb.countProjection(candidate.getHead(),
										candidate.getAntecedent());
							}
	...
	/**
	 * It determines whether the rule contains an expensive query patterns of the
	 * forms
	 * #(x, y) : r(z, x) r(z, y) or #(x, y) : r(y, z) r(x, z). Such query patterns
	...
	protected boolean containsHardCase(Rule query, int[] newEdge) {
		...
		if (this.kb.identifyHardQueryTypeI(subquery) != null)
			return true;
	...
	/**
	 * Application of the "Add instantiated atom" operator. It takes a rule of the
	 * form
	 * r(x, w) ^ ..... =&gt; rh(x, y), where r(x, w) is recently added atom and adds
	 * to the
	 * output all the derived rules where "w" is bound to a constant that keeps the
	 * whole
	...
	protected void getInstantiatedAtoms(Rule query, Rule parentQuery,
		...
		if (rewrittenQuery != null) {
			long t1 = System.currentTimeMillis();
			constants = this.kb.countProjectionBindings(rewrittenQuery.getHead(), rewrittenQuery.getAntecedent(),
					danglingEdge[danglingPosition]);
			long t2 = System.currentTimeMillis();
			if ((t2 - t1) > 20000 && this.verbose)
				System.err.println("countProjectionBindings var=" + kb.unmap(danglingEdge[danglingPosition]) + " in "
						+ query + " (rewritten to " + rewrittenQuery + ") has taken " + (t2 - t1) + " ms");
		} else {
			long t1 = System.currentTimeMillis();
			constants = this.kb.countProjectionBindings(query.getHead(), query.getAntecedent(),
					danglingEdge[danglingPosition]);
			long t2 = System.currentTimeMillis();
			if ((t2 - t1) > 20000 && this.verbose)
				System.err.println("countProjectionBindings var=" + kb.unmap(danglingEdge[danglingPosition]) + " in "
						+ query + " has taken " + (t2 - t1) + " ms");
		}
	 ...
	 * so that they become less expensive to evaluate. This function targets
	 * exclusively queries
	 * of the form:
	 * r(x, z) r(x, w) =&gt; r'(x, y)
	 * where the newly added atom r(x, z) does not really make the query more
	 * selective and it is
	 * therefore redundant.
	 * 
	 * @param query
		...
		if (hardnessInfo != null) {
			...
			if (toRemove == null) {
				...
				if (!query.variableCanBeDeleted(victimTriplePos, victimVarPos)) {
					...
					rewrittenQuery = query.rewriteQuery(toRemove, target, toRemove[victimVarPos],
							query.getTriples().get(targetTriplePos)[victimVarPos]);
				}
			}
		}
	...
	/**
	 * Returns the number of distinct bindings of the given variables in the body of
	 * the rule.
	 * 
	 * @param var1
	 * @param var2
	...
	/**
	 * Returns the denominator of the PCA confidence expression for the antecedent
	 * of a rule.
	 * 
	 * @param var1
	 * @param var2
	...
	public static void main(String[] args) throws IOException {
		KB kb = new KB();
		// db.load(new
		// File("/home/galarrag/workspace/AMIE/Data/yago2s/yagoFacts.decoded.compressed.ttl"));
		kb.load(new File(
				"/home/galarrag/workspace/AMIE/Data/yago2/yago2core.decoded.compressed.notypes.nolanguagecode.tsv"));
		...
		System.out.println("Results Std: "
				+ kb.countDistinctPairs(kb.map("?a"), kb.map("?b"), pcaDenom.subList(1, pcaDenom.size() - 1)));
