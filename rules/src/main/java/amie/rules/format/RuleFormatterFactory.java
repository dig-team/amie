...
public class RuleFormatterFactory {
	...
	public static RuleFormatter getFormatter(String className, boolean verbose, Map<String, Object> args) throws ClassNotFoundException, 
		...
		if (className.toLowerCase().equals("default") || className.equals("amie.rules.format.DefaultRuleFormatter")) {
			if (args.containsKey("ommitStd")) {
				if (args.get("ommitStd").equals(Boolean.TRUE)) {
					return new NoStdDefaultRuleFormatter(verbose);
				}
			}
